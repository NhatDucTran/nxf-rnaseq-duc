...
def check_design(DesignFileIn, DesignFileOut, DEseq2GroupLabels, Comparison, Kitqcplotdatasets, IgnoreR1):
    """
    ...
    if IgnoreR1:
        assert design["read_2"].map(check_fastq_suffix).all()
    else:
    ...
    if Kitqcplotdatasets is not None:
        kitqc = pd.read_csv(Kitqcplotdatasets, index_col=None)
        HEADER = ['library_1','library_2']
        assert list(kitqc.columns) == HEADER, "Header of Kit QC file must be {}!".format(','.join(HEADER))
        assert kitqc['library_1'].isin(design['sample']).all(), "Some labels in library_1 column not present in design file!"
        assert kitqc['library_2'].isin(design['sample']).all(), "Some labels in library_2 column not present in design file!"
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", dest="Kitqcplotdatasets", help="Scatterplot datasets CSV file, only for RiboFree Kit QC")
    ...
    check_design(args.DesignFileIn, "checked_"+args.DesignFileIn, "DESeq2_"+args.DesignFileIn, args.Comparison, args.Kitqcplotdatasets, args.IgnoreR1)