...
process deliver_fastqs {
    ...
    output:
    path "${meta.name}_*.fastq.gz", includeInputs: true, emit: downloads
    path "${meta.name}_fastq_md5sum.txt", emit: md5sum

    script:
    if (meta.single_end) {
        """
        [ ! -f ${meta.name}_R1.fastq.gz ] && ln -s $reads ${meta.name}_R1_fastq.gz
        md5sum ${meta.name}_*.fastq.gz > ${meta.name}_fastq_md5sum.txt
        """
    }
    else {
        """
        [ ! -f ${meta.name}_R1.fastq.gz ] && ln -s ${reads[0]} ${meta.name}_R1_fastq.gz
